...
export default function MyReports() {
  ...
  const fetchChatMessages = async (reportId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/reports/${reportId}/messages`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      const attachments = selectedImages.map(img => img.preview);

      const response = await fetch(`http://localhost:5000/api/reports/${selectedReport.id}/messages`, {
        method: 'POST',
        headers: {
  ...
  return (
    <>
      <Head>
        <title>My Reports - GWorker</title>
      </Head>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Track and communicate about your reports</p>
                </div>
              </div>
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-500 mt-2">
                        <span className="flex items-center space-x-1">
                          ...
                          <span>{report.message_count || 0} messages</span>
                        </span>
                        <span>{new Date(report.created_at).toLocaleDateString()}</span>
                      </div>

                      {report.unread_admin_messages > 0 && (
                        <div className="mt-2">
                          <span className="inline-flex items-center px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">
                            {report.unread_admin_messages} new message{report.unread_admin_messages > 1 ? 's' : ''}
                          </span>
                        </div>
                      )}
                    </div>
            ...
            <div className="lg:col-span-2 bg-white rounded-lg shadow">
              {!selectedReport ? (
                <div className="p-8 text-center text-gray-500">
                  ...
                  <p>Select a report to view details and chat with admin</p>
                </div>
              ) : (
                <>
                  {/* Report Details */}
                  <div className="p-4 border-b">
                    ...
                    <div className={`inline-flex items-center space-x-1 px-3 py-1 rounded-full text-sm mb-3 ${getStatusBadge(selectedReport.status)}`}>
                      {getStatusIcon(selectedReport.status)}
                      <span className="capitalize font-medium">Status: {selectedReport.status}</span>
                    </div>
                    ...
                    <div className="text-xs text-gray-500 space-y-1">
                      ...
                      {selectedReport.resolved_at && (
                        <p><strong>Resolved:</strong> {new Date(selectedReport.resolved_at).toLocaleString()}</p>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Send Message / Ticket Closed Notice */}
                  {selectedReport.status === 'resolved' || selectedReport.status === 'dismissed' ? (
                    <div className="p-4 border-t bg-gray-100">
                      <div className="flex items-center justify-center space-x-2 text-gray-600">
                        <FiXCircle className="w-5 h-5" />
                        <span className="font-medium">
                          This ticket has been {selectedReport.status === 'resolved' ? 'resolved' : 'dismissed'} and is now closed.
                        </span>
                      </div>
                      <p className="text-center text-sm text-gray-500 mt-2">
                        You cannot send messages on closed tickets.
                      </p>
                    </div>
                  ) : (
                    <form onSubmit={sendMessage} className="p-4 border-t">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <button
                          ...
                          disabled={sendingMessage || selectedImages.length >= 5}
